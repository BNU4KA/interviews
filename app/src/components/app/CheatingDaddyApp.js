...
export class App extends LitElement {
    ...
    constructor() {
        ...
        this.selectedProfile = localStorage.getItem('selectedProfile') || 'leetcode';
    ...
    async handleStart() {
        await cheddar.initializeGemini(this.selectedProfile || 'leetcode', this.selectedLanguage);
    ...
    async handleSendImage(imageBase64, question = '') {
        const result = await window.cheddar.sendImageMessage(imageBase64, question);

        if (!result.success) {
            console.error('Failed to send image:', result.error);
            this.setStatus('Error sending image: ' + result.error);
        } else {
            this.setStatus('Image sent...');
            this._awaitingNewResponse = true;
        }
    }
            ...
            case 'assistant':
                return html`
                    <assistant-view
                        ...
                        .onSendImage=${(imageBase64, question) => this.handleSendImage(imageBase64, question)}
