...
export class InterviewApp extends LitElement {
    ...
    async _loadFromStorage() {
        try {
            if (!window.interviewApp || !window.interviewApp.storage) {
                console.error({ error: 'interviewApp.storage is not available for initial load' });
                this._storageLoaded = true;
                this.requestUpdate();
                return;
            }

            const [config, prefs] = await Promise.all([window.interviewApp.storage.getConfig(), window.interviewApp.storage.getPreferences()]);
    ...
    async _reloadSettingsFromStorage() {
        try {
            if (!window.interviewApp || !window.interviewApp.storage) {
                console.error({ error: 'interviewApp.storage is not available for reload' });
                return;
            }
            ...
            console.error({ error: 'Error reloading settings from storage', message: error.message, stack: error.stack });
        }
    }
    ...
    async handleClose() {
        if (this.currentView === 'customize' || this.currentView === 'help' || this.currentView === 'history') {
            ...
            window.interviewApp.stopCapture();
    ...
    async handleStart() {
        console.log({ handleStart: { selectedProfile: this.selectedProfile, selectedLanguage: this.selectedLanguage } });
        if (!window.interviewApp) {
            console.error({ error: 'window.interviewApp is not available' });
            return;
        }
        console.log({ message: 'Initializing session...' });
        await window.interviewApp.initializeSession(this.selectedProfile, this.selectedLanguage);
        console.log({ message: 'Starting capture...' });
        window.interviewApp.startCapture(this.selectedScreenshotInterval, this.selectedImageQuality);
        this.responses = [];
        this.currentResponseIndex = -1;
        this.startTime = Date.now();
        this.currentView = 'assistant';
        console.log({ message: 'Start completed, switching to assistant view' });
    }
    ...
    async handleScreenshotIntervalChange(interval) {
        ...
        if (window.interviewApp && window.interviewApp.storage) {
            await window.interviewApp.storage.updatePreference('selectedScreenshotInterval', interval);
        }
    }
    ...
    async handleImageQualityChange(quality) {
        ...
        if (window.interviewApp && window.interviewApp.storage) {
            await window.interviewApp.storage.updatePreference('selectedImageQuality', quality);
        }
    }
    ...
    async handleSendText(message) {
        const result = await window.interviewApp.sendTextMessage(message);
            ...
            case 'customize':
                return html`
                    <customize-view
                        ...
                        .onSaveClick=${() => this.handleSaveSettings()}
                    ></customize-view>
                `;
    ...
    async handleLayoutModeChange(layoutMode) {
        ...
        if (window.interviewApp && window.interviewApp.storage) {
            await window.interviewApp.storage.updateConfig('layout', layoutMode);
        }
        this.updateLayoutMode();
    ...
    async handleSaveSettings() {
        console.log({ handleSaveSettings: { currentView: this.currentView } });

        if (this.currentView !== 'customize') {
            console.log({ error: 'Not in customize view' });
            return;
        }
        ...
        try {
            ...
            const findCustomizeView = () => {
                if (this.shadowRoot) {
                    const viewContainer = this.shadowRoot.querySelector('.view-container');
                    if (viewContainer) {
                        const found = viewContainer.querySelector('customize-view');
                        if (found) return found;
                    }
                    const found = this.shadowRoot.querySelector('customize-view');
                    if (found) return found;
                }
                return document.querySelector('customize-view');
            };

            const customizeView = findCustomizeView();
            console.log({ customizeViewFound: !!customizeView, hasSaveAll: customizeView && typeof customizeView.saveAll === 'function' });

            if (customizeView && typeof customizeView.saveAll === 'function') {
                try {
                    await customizeView.saveAll();
                    console.log({ saveAllCompleted: true });

                    await new Promise(resolve => setTimeout(resolve, 100));
                    await this._reloadSettingsFromStorage();
                    ...
                    console.error({ error: 'Error in saveAll', message: saveError.message, stack: saveError.stack });
                    throw saveError;
                }
            } else {
                const errorMsg = 'CustomizeView not found or saveAll method missing';
                console.error({ error: errorMsg, customizeView, shadowRoot: !!this.shadowRoot });
                throw new Error(errorMsg);
            }
        } catch (error) {
            console.error({ error: 'Error saving settings', message: error.message, stack: error.stack });
            this._showSaved = false;
        } finally {
            this._isSaving = false;
...
customElements.define('interview-app', InterviewApp);
