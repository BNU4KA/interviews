...
export class AppHeader extends LitElement {
    ...
    async _checkForUpdates() {
        try {
            if (window.interviewApp) {
                const currentVersion = await window.interviewApp.getVersion();
            }
        } catch (err) {
            console.log({ error: 'Update check failed', message: err.message });
        }
    }
    ...
    async _openUpdatePage() {
        if (window.require) {
            try {
                ...
                await ipcRenderer.invoke('open-external', 'https://github.com/sohzm/cheating-daddy/releases');
            } catch (err) {
                console.log({ error: 'Failed to open update page', message: err.message });
            }
        }
    }
    ...
    getViewTitle() {
        const titles = {
            ...
            assistant: 'Interview App',
        };
        return titles[this.currentView] || 'Interview App';
    }
                        ...
                        ? html`
                              <button @click=${this.onHideToggleClick} class="button">
                                  Hide&nbsp;&nbsp;<span class="key" style="pointer-events: none;"
                                      >${window.interviewApp?.isMacOS ? 'Cmd' : 'Ctrl'}</span
                          ...
                          `
                        : html`
                              ${this.currentView === 'customize' && (this.hasUnsavedChanges || this.showSaved)
                                  ? html`
                                        <button
                                            @click=${e => {
                                                e.preventDefault();
                                                ...
                                                if (!this.isSaving && !this.showSaved && this.onSaveClick) {
                                                    this.onSaveClick();
                                                }
                                            }}
                                            class="save-button ${this.showSaved ? 'saved' : ''}"
                                            ?disabled=${this.isSaving || this.showSaved}
                                        >
                                            ${this.showSaved ? 'Saved' : 'Save Changes'}
                                        </button>
                                    `
                                  : ''}
