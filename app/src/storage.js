...
function getConfigDir() {
    ...
    if (platform === 'win32') {
        configDir = path.join(os.homedir(), 'AppData', 'Roaming', 'interview-app-config');
    } else if (platform === 'darwin') {
        configDir = path.join(os.homedir(), 'Library', 'Application Support', 'interview-app-config');
    } else {
        configDir = path.join(os.homedir(), '.config', 'interview-app-config');
    }
...
function getPreferences() {
    ...
    const result = { ...DEFAULT_PREFERENCES, ...saved };
    console.log({
        storageGetPreferences: {
            saved,
            result,
...
function updatePreference(key, value) {
    ...
    const result = writeJsonFile(getPreferencesPath(), preferences);
    console.log({
        storageUpdatePreference: {
            key,
            oldValue,
            newValue: value,
            success: result,
